...
function app() {
  ...
  var chatBar = {
      ...
      create: function () {
        ...
        log.style.transform =
          "translateY(" + (-this.barH * 2 - this.logH) + "px)"
    ...
    cmd = [
      ...
      new cmdObj(
        "modentity",
        "<name> <newname> [gender] [skin] [speed] [level]",
        "modify entity"
      ),
      new cmdObj(
        "npc",
        "<add|del> <name> [gender] [skin] [speed] [level] [<x> <y>]",
        "add/delete NPC"
      ),
      new cmdObj(
        "tp",
        "<name> <x> <y> or <name> <targetname>",
    ...
    avatar = function (
      ...
      let nameLenLimit = 16
      this.name =
        name.length > nameLenLimit
          ? name.substr(0, nameLenLimit)
          : name || "Anonymous"
        ...
        if (msg.length > 0) {
          ...
          if (this.lvl === 0 && isCmd) {
            switch (msg.substr(1, msg.length - 1).split(" ")[0]) {
              ...
              case "help":
                ...
                screenText.updateText(
                  helpScrnTxt,
                  h - chatBar.barH - screenText.fontS * 1.5 * cmdInfo.length,
                  screenText.fontS * 2 * cmdInfo.length,
                  "#4f4"
                )
                break
              ...
              case "entityinfo":
                ...
                if (eiSearch !== 0 && eiTarget) {
                  ...
                  eiFBLines[1] =
                    "Gender - " + (eiSearch.gender === 0 ? "male" : "female")
                  eiFBLines[2] = "Skin - " + eiSearch.skinTone
                  eiFBLines[3] = "Speed - " + eiSearch.speed
                  eiFBLines[4] =
                    "Coordinates - " +
                    Math.round(eiSearch.x) +
                    "," +
                    Math.round(eiSearch.y)
                ...
                screenText.updateText(
                  eiFeedback,
                  h - chatBar.barH - screenText.fontS * 1.5 * (eiFBLen - 1),
                  screenText.fontS * 2 * (eiFBLen - 1),
                  eiSearch !== 0 && eiArgs[1] ? "#ff4" : "#f44"
                )
                break
                ...
                if (meTarget) {
                  if (meSearch !== 0) {
                    if (meName) {
                      ...
                      let meNameUsed =
                        worldObjs.find((ne) => ne.name === meName) || 0
                      if (meNameUsed === 0 || meTarget == meNameUsed.name) {
                        if (meGender) {
                          if (
                            (meGender >= 0 && meGender <= 1) ||
                            meGender == "male" ||
                            meGender == "m" ||
                            meGender == "female" ||
                            meGender == "f"
                                    ...
                                    if (meLevel) {
                                      if (meLevel >= 0 && meLevel <= 20) {
                                        ...
                                        if (meTarget == player.name) {
                                          meInvalid = true
                                          meFeedback =
                                            "Entity must be an NPC to modify AI activity level."
                                        }
                                      } else {
                                        meInvalid = true
                                        meFeedback =
                            ...
                            meInvalid = true
                            meFeedback =
                              "Gender must be 0 or 1. m(ale) and f(emale) are also valid."
                          }
                        }
                      } else {
                        meInvalid = true
                        meFeedback =
                          "'" +
                          meNameUsed.name +
                          "' has already been used. Please choose another name."
                      }
                    } else {
                      meInvalid = true
                  ...
                  meInvalid = true
                  meFeedback =
                    "Usage: /modentity <name> <newname> [gender] [skin] [speed] [level]"
                }

                if (!meInvalid) {
                  let nameLenLimit = 16
                  meSearch.name =
                    meName.length > nameLenLimit
                      ? meName.substr(0, nameLenLimit)
                      : meName
                  if (meValidArgCt >= 3)
                    meSearch.gender =
                      meGender == "male" || meGender == "m"
                        ? 0
                        : meGender == "female" || meGender == "f"
                        ? 1
                        : meGender
                  if (meValidArgCt >= 4) meSearch.skinTone = meSkin
                  if (meValidArgCt >= 5) meSearch.speed = +meSpeed
                  if (meValidArgCt == 6) meSearch.lvl = +meLevel
                }
              ...
              case "npc":
                let npcArgs = msg.split(" "),
                  ...
                  npcInvalid = false,
                  npcUsage =
                    "Usage: /npc <add|del> <name> [gender] [skin] [speed] [level] [<x> <y>]"

                if (npcAction == "add") {
                  if (npcName) {
                    let npcNameUsed =
                      worldObjs.find((np) => np.name === npcName) || 0
                    if (npcNameUsed === 0) {
                      if (npcGender) {
                        if (
                          (npcGender >= 0 && npcGender <= 1) ||
                          npcGender == "male" ||
                          npcGender == "m" ||
                          npcGender == "female" ||
                          npcGender == "f"
                                        ...
                                        if (npcY) {
                                          if (!isNaN(npcX) && !isNaN(npcY)) {
                                            ...
                                            if (
                                              ...
                                              npcInvalid = true
                                              npcFeedback =
                                                "Placement is out of bounds. X limit is 0-" +
                                                xMax +
                                                ". Y limit is 0-" +
                                                (h - chatBar.barH) +
                                                "."
                                            ...
                                            npcInvalid = true
                                            npcFeedback =
                                              "Placement coordinates are invalid."
                                          }
                                        } else {
                                          npcInvalid = true
                                          npcFeedback =
                                            "Placement coordinates need both X and Y."
                                        }
                                      }
                                    } else {
                                      npcInvalid = true
                  ...
                  if (!npcInvalid) {
                    let aGender =
                        (npcGender == "male" || npcGender == "m"
                          ? 0
                          : npcGender == "female" || npcGender == "f"
                          ? 1
                          : npcGender) || 0,
                      aSkin = npcSkin || 0,
                      aSpeed = npcSpeed || 3,
                ...
                if (tpAfterEn) {
                  if (isNaN(tpAfterEn) && tpAfterEn[0] != rel) {
                    let tarEntity = tpAfterEn,
                      tEnSearch =
                        worldObjs.find((ts) => ts.name === tarEntity) || 0,
                      bothNames = tpEntity && tarEntity ? true : false

                    tpOK =
                      bothNames && enSearch !== 0 && tEnSearch !== 0
                        ? true
                        : false
                    tpFeedback = bothNames
                      ? enSearch !== 0
                        ? tEnSearch !== 0
                          ? "Teleported " + tpEntity + " to " + tarEntity
                    ...
                    ;(tpOK =
                      enSearch !== 0 && allValues && wthnScrn ? true : false),
                      (tpFeedback = allValues
                        ? enSearch !== 0
                          ? wthnScrn
                            ? "Teleported " +
                              tpEntity +
                              ...
                              "," +
                              Math.round(tpY)
                            : "Coordinates are out of bounds. X limit is 0-" +
                              cw +
                              ". Y limit is 0-" +
                              (h - chatBar.barH) +
                              "."
            ...
            newEntry.appendChild(
              document.createTextNode(this.name + ": " + this.lastMsg)
            )
          }
          // add new line
          ...
          if (chatLog.childNodes.length > chatBar.maxLines) {
            chatLog.removeChild(
              chatLog.getElementsByTagName("span")[chatBar.maxLines]
            )
          }
        }
      }
    ...
    structure = function (width, height, x, y, backArea, img, isAnim, frames) {
      ...
      this.img = img || null
      this.isAnim =
        img && isAnim ? (typeof isAnim == "boolean" ? true : false) : false
      this.frames = frames || 1
      this.curFrame = 1
    },
    ...
    createNPCs = function () {
      ...
      for (var npcn in npcNames) {
        ...
        if (findCllsn(npcs[npcn], structures)) {
          ...
          npcs[npcn].x = player.x + 90
          npcs[npcn].y = player.y + 90
        }
      }
    },
    ...
    drawAvatar = function (avatar) {
      ...
      if (lastMsg.length > 0 && avatar.msgTimer > 0) {
        ...
        for (var lm in strS) {
          ...
          lm = +lm
          line[l] +=
            strS[lm] + (lm != strS.length - 1 && !isNotLatin ? " " : "")
        ...
        let wMult = !isNotLatin ? 0.7 : 1.2,
          bubble = new bubbleObj(
            lastMsg,
            longestLnLen * fontS * wMult,
            avatar.x + avatar.w / 2,
            avatar.y - avatar.h - 35
          )
        ...
        for (var bl in line) {
          bl = +bl
          ctx.fillText(
            line[line.length - 1 - bl],
            bubble.x,
            bubbleY + bottomLnSt - fontS * 1.2 * bl
          )
        }
        ctx.globalAlpha = 1
      ...
      ctx.drawImage(
        ...
        avatar.w * (avatar.curFrame - 1) +
          avatar.w * avatar.frames * avatar.dir,
        ...
        if (!chatBar.showLog) {
          ...
          for (var l in lines) {
            ctx.fillText(
              lines[l],
              5,
              txtObj.y - adj - txtObj.fontS * 1.5 * -(l - 1)
            )
          }
          ctx.globalAlpha = 1
        }
  ...
  document.addEventListener("keydown", function (e) {
    ...
    if (!chatBar.active) {
      ...
      if (
        e.keyCode == 38 &&
        chatBar.curHistoryItem != chatBar.history.length - 1
      ) {
        ++chatBar.curHistoryItem
        field.value =
          chatBar.history[chatBar.history.length - chatBar.curHistoryItem - 1]
        ...
        field.value =
          chatBar.curHistoryItem == -1
            ? ""
            : chatBar.history[
                chatBar.history.length - chatBar.curHistoryItem - 1
              ]
      }
    }
    ...
    if (e.keyCode == 9 && field.value[0] == "/" && chatBar.active) {
      ...
      if (chatBar.autoCmpltLvl === 0) {
        ...
        for (var fm in cmd) {
          if (
            cmd[fm].name.indexOf(curString) === 0 &&
            curString.length >= 1 &&
            curString != cmd[fm].name
          ) {
        ...
        if (
          ...
          (cmd.find((c) => c.name == curString) || field.value == "/")
        ) {
        ...
        if (curCmd !== 0 && reqArgs >= 1) {
          ...
          if (cmdInHand.args.indexOf("name") == 1) {
            ...
            cmdInHand.args.indexOf("add") == 1 ||
            cmdInHand.args.indexOf("del") == 1
          ) {
